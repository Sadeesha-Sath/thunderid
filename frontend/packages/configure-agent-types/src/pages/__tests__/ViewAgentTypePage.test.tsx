...
vi.mock('@/components/edit-agent-type/schema-settings/EditSchemaSettings', () => ({
  default: ({onPropertiesChange}: {onPropertiesChange: (props: unknown[]) => void}) => (
    <div data-testid="edit-schema-settings">
      ...
      {/* Emits the exact shape convertSchemaToProperties(baseAgentType.schema) produces, so the
          page should treat it as "no change" (same key order matters for the JSON compare). */}
      <button
        type="button"
        onClick={() =>
          onPropertiesChange([
